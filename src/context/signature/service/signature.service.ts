...
import { MeetingPayload, VideoPayload } from '../domain/payload';
...
export class SignatureService {
  ...
  signatureMeeting(request: SignatureMeetingRequest): SignatureResponse {
  ...
  signatureVideo(request: SignatureVideoRequest): SignatureResponse {
    const iat = Math.floor(Date.now() / 1000);
    const exp = request.expirationSeconds
      ? iat + request.expirationSeconds
      : iat + 60 * 60 * 2;
